...
class DatasetHandler:
    ...
    def vis_segmentation(self, segmented_image):
        ...
        colored_segmentation[segmented_image ==
                             BUILDING_FLAG] = self.colour_mappings['Buildings']
        colored_segmentation[segmented_image ==
                             PEDESTRIAN_FLAG] = self.colour_mappings['Pedestrians']
        colored_segmentation[segmented_image ==
                             POLES_FLAG] = self.colour_mappings['Poles']
        colored_segmentation[segmented_image ==
                             LANE_FLAG] = self.colour_mappings['RoadLines']
        colored_segmentation[segmented_image ==
                             ROAD_FLAG] = self.colour_mappings['Roads']
        colored_segmentation[segmented_image ==
                             SIDEWALK_FLAG] = self.colour_mappings['Sidewalks']
        colored_segmentation[segmented_image ==
                             VEHICLE_FLAG] = self.colour_mappings['Vehicles']
...
def get_slope_intercept(lines):
    if len(lines) == 0:
        return [], []
    print('get_slope_intercept> lines shape: {}, lines: {}'.format(lines.shape, lines))
